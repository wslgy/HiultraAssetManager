...
import com.hiultra.assetManagerNeutral.adapter.StopAdapter;
 ...
 * @Time 上午11:30:03
 */
public class AssetStopFragment extends AsyncLoadFragment {
    ...
    private Asset_StopInfo01 info01;
    private ArrayList<Asset_StopInfo02> info02List = new ArrayList<>();
    
    
    @Override
    protected View getSuccessView(LayoutInflater inflater) {
        View v = inflater.inflate(R.layout.layout_stop, null);
    ...
    private void initLv() {
        adapter = new StopAdapter(mActivity, info02List);
    ...
    protected Object loadData() {
        info02List.clear();
        info01 = DBTools.findFirst(Selector.from(Asset_StopInfo01.class).where("ifuodate", "=", null));
        if (info01 == null) {
            info01 = new Asset_StopInfo01();
        ...
        ArrayList<Asset_StopInfo02> list = DBTools.findAll(Asset_StopInfo02.class, "BatchNumber", info01.getBatchNumber());
        if (list != null && !list.isEmpty()) {
            info02List.addAll(list);
        }
        for (Asset_StopInfo02 info : info02List) {
    ...
    public void onEpcRead(String epc) {
        ...
        for (int i = 0; i < info02List.size(); i++) {
            Asset_StopInfo02 info02 = info02List.get(i);
        ...
        Asset_StopInfo02 info = new Asset_StopInfo02();
    ...
    @OnClick(R.id.btn_confirmStop)
    private void confirmStop(View v) {
        if (info01 == null) {
        ...
        builder.setPositiveButton(R.string.msg_button_ok, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                info01.setStopDate(date);
                info01.setRemark(etRemark.getText().toString().trim());
                for (Asset_StopInfo02 info02 : info02List) {
    ...
    protected void updateAsset(Asset_StopInfo01 info01, Asset_StopInfo02 info02) {
    ...
    private class LvListener implements OnItemClickListener, OnDismissCallback {
        ...
        public void onDismiss(int dismissPosition) {
            Asset_StopInfo02 item = adapter.getItem(dismissPosition);
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            Asset_StopInfo02 item = adapter.getItem(position);
    ...
    public void showDetail(final Asset_StopInfo02 item, AssetMaterialInfo a) {
        AlertDialog.Builder builder = new AlertDialog.Builder(mActivity);
