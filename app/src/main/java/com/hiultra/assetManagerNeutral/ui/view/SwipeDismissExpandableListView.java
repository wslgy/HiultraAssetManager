...
/**
 * 带侧滑删除功能的ExpandableListView
 * @author Tom
 * @date 2016年9月21日
 * @Time 下午5:15:49
 */
public class SwipeDismissExpandableListView extends ExpandableListView {
    ...
    public SwipeDismissExpandableListView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SwipeDismissExpandableListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void handleActionDown(MotionEvent ev) {
        ...
        long mDownExpandPosition = getExpandableListPosition(mDownPosition);
        int groupPosition = getPackedPositionGroup(mDownExpandPosition);
        int childPosition = getPackedPositionChild(mDownExpandPosition);
        if(childPosition == -1) // 父条目不可被滑动
            return;

        // TODO mDownPosition - headerCount < 0实现了头布局不可侧滑
        if (mDownPosition - headerCount < 0 || mDownPosition == AdapterView.INVALID_POSITION ) {
            return;
        }
    ...
    private void performDismiss(final View dismissView, int dismissPosition) {
        /* 获取Group以及Child的索引 */
        long position = getExpandableListPosition(dismissPosition);
        final int groupPosition = getPackedPositionGroup(position);
        final int childPosition = getPackedPositionChild(position);
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (onDismissCallback != null) {
                    onDismissCallback.onDismiss(groupPosition, childPosition);
                }
    ...
    public interface OnDismissCallback {
        public void onDismiss(int groupPosition, int childPosition);
    }
